...
namespace Mobius
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            {
                Console.WriteLine($"错误:无法找到{ffmpegPath}.请检查配置文件.");
                return;
            }
            else if (args.Length == 0)
            {
                Console.WriteLine("错误:请选择要转换的.mods或.moflex文件");
                return;
            }
            else
            {
                ...
                {
                    Console.WriteLine($"正在转换{path}...");
                    try
                    {
                        var stp = Stopwatch.StartNew();
                        Transcode(path, path + ".mp4");
                        stp.Stop();
                        Console.WriteLine($"转换完成,耗时:{stp.Elapsed:mm\\:ss\\.ff}");
                    }
                    catch (Exception e)
                    {
